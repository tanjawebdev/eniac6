...
export function DebugOverlay({ standalone = false }: DebugOverlayProps) {
  ...
  useEffect(() => {
    ...
    THEME_IDS.forEach((themeId) => {
      [0, 1].forEach((socketIndex) => {
        ...
        if (nextMappings[mappingKey] === undefined) {
          if (activeProg) {
  ...
  return (
    <div className={`debug-overlay glass-panel ${standalone ? 'standalone' : ''}`}>
      ...
      <div className="debug-overlay-content">
        <div className="debug-overlay-col">
          {/* Network / Mode Info */}
